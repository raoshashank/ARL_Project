...
for i in range(1,num_units+1):
    error_code_h, joints_h_handles[i-1] = vrep.simxGetObjectHandle(clientID,'snake_joint_h'+str(i),vrep.simx_opmode_oneshot_wait)
    error_code_v, joints_v_handles[i-1] = vrep.simxGetObjectHandle(clientID,'snake_joint_v'+str(i),vrep.simx_opmode_oneshot_wait)
...
while True:
    ...
    for i in range(1,5):
        ...
        err_code_h = vrep.simxSetJointTargetPosition(clientID,joints_h_handles[i-1],h_cmd,vrep.simx_opmode_oneshot)
        v_cmd =(A_V[0]*(1-s)+A_V[1]*s)*math.sin(t*(speed[0]*(1-s)+speed[1]*s)+i*(P_V[0]*(1-s)+P_V[1]*s))
        err_code_v = vrep.simxSetJointTargetPosition(clientID,joints_v_handles[i-1],v_cmd,vrep.simx_opmode_oneshot)
